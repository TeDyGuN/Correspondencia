...
<html>
  ...
  <body>
      ...
      <div width="100%" style="margin-top:70px;">
        <h3 class="text-center" style="text-decoration: underline;"><strong>SOLICITUD DE VACACIONES</strong></h3>
      </div>
    </div>
    <div class="">
    ...
    <div class="">
      <p style="text-decoration: underline;"><strong>Vacaciones Previstas</strong></p>
      <table width="100%" style="margin-bottom: 10px;">
        <tr>
          <td width="10%"><strong>De:</strong></td>
          <td width="25%">{{ $carbon = Carbon\Carbon::parse($reporte[0]->inicio)->format('d/m/Y') }}</td>
          <td width="10%"><strong>Hasta:</strong></td>
          <td width="25%">{{ $carbon = Carbon\Carbon::parse($reporte[0]->fin)->format('d/m/Y') }}</td>
          <td width="20%"><strong>Duración (Días):</strong></td>
          <td width="15%"> {{ $reporte[0]->duracion }}</td>
        </tr>
      </table>
      <p><strong>Ultimo día de Trabajo:</strong> {{ $carbon = Carbon\Carbon::parse($reporte[0]->ultimo_trabajo)->format('d/m/Y') }} </p>
      <p><strong>Reaundación de trabajo prevista para:</strong> {{ $carbon = Carbon\Carbon::parse($reporte[0]->reanudacion_trabajo)->format('d/m/Y') }} </p>
