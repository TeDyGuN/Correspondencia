        ...
        <div class="row" >
            <div class="">
                <div class="panel panel-default" id="panel-profin">
                    <div class="panel-heading text-center textoHeader">Modificacion de Corrrespondencia Enviada</div>
                    <div class="panel-body">
                        ...
                        @endif
                            <form class="form-horizontal" role="form" method="POST" action="{{ url('/enviados/editar/guardar') }}" enctype="multipart/form-data">
                                {{ csrf_field() }}
                                <input type="hidden" name="id" value="{{ $aso->id }}">
                                <div class="form-group{{ $errors->has('emitente') ? ' has-error' : '' }}">
                                    <label for="emitente" class="col-md-4 control-label">Emitente</label>

                                    <div class="col-md-6">
                                        <input id="emitente" type="text" class="form-control" name="emitente" value="{{ $aso->emitente }}">

                                        @if ($errors->has('emitente'))
                                            <span class="help-block">
                                        <strong>{{ $errors->first('emitente') }}</strong>
                                    </span>
                                        @endif
                                    </div>
                                </div>
