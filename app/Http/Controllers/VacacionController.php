...
class VacacionController extends Controller
{
    ...
    public function saveAsignacion(Request $request){
      $v = new Asignacion();
      $v->id_user = $request->usuario;
      $v->dias = $request->duracion;
      $v->motivo = $request->obs;
      $v->save();
      $idq = Asignacion::orderBy('created_at', 'desc')->first();
      $id = $idq->id;
    ...
    //   $ani$reporte = Horario::join('users as u', 'id_user', 'u.id')
    //               ->whereYear('fecha', $anio)
    //               ->whereMonth('fecha', $mes)
    //               ->paginate(15);
    //   return view('Asistencia/profinReporte', compact('reporte', 'anio', 'mes'));o = ['2018'];
    //   $mes = ['Febrero'];
